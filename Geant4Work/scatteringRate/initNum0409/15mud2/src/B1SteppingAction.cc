...
int eventIDflag[200000] = {200001};  //重复计数排除数组
int eventIDconvflag[200000] = {200001};  // 对发生电子对效应的粒子进行统计
// G4int eventIDmax = 0;
G4int flagcodeRP = 0;
  ...
  if (nextpv!=NULL)
  {
    ...
    if(nextvolumename=="Shape2")
    {
      ...
      if (E == 2)
      {
        ...
        if ((eventID > 100000)&&(eventID <= 200000))
        {
          jx = jx + 1;
        }
      }
    ...
    if ((eventID == 199999) & (outputcode == 10))
    {
      outputcode = eventID; //使eventID最大时只输出一次，不重复输出。
      G4cout<<"EffectiveCountallnoconv1:"<< i <<G4endl;
      G4cout<<"EffectiveCountallnoconv1:"<< i1 <<G4endl;
      G4cout<<"EffectiveCountnoscattering:"<< j <<G4endl;
      G4cout<<"EffectiveCountnoscattering:"<< jx <<G4endl;
